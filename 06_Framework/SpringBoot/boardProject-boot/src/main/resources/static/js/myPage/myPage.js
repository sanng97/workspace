...
if (imageInput != null) { // #imageInput 존재할 때
    ...
    const changeImageFn = e => {
        ...
        console.log(e.target.value); // 업로드 파일 경로 fakepath로 출력(정책상 경로 알수없음)

        /* 이게 중요!! */
        console.log(e.target.files); // 업로드된 파일의 정보가 담긴 배열 반환
        // * 실제 파일 *

        console.log(e.target.files[0]); // 업로드된 파일 중 첫번째 파일
        ...
        if (uploadFile.size > maxSize) {
            ...
            if(imageInput.value== -1){ // 이미지 변경이 없었을 때

 
            
            // 최대 크기를 초과해도 input에 value가 남기 때문에
            // 이를 제거하는 코드가 필요하다!
            imageInput.value = ''; // value 삭제 -> 동시에 files도 삭제됨


          
                statusCheck = -1; // 선택없음 상태
