...
boardLike.addEventListener("click",e => {
  ...
  // 1. 로그인 여부확인 (로그인한 회원만 좋아요를 누를 수 있다)

  // *** 로그인 여부 확인 방법 ***
  // session에 loginMember가 있으면 로그인 상태
  // 없으면 로그아웃
  // -> session을 이용해야 되는데 js만으로는 server session 사용 불가
  //    단 , Thymeleaf를 작성할 수 있는 html 파일에서는 js와 혼용가능
  //    * html 파일에 boardDetail.js를 추가하기 전에
  //      script 태그를 이용해서 전역변수로 로그인 여부를 저장*
      ...
      /* 목록으로 버튼 */
        const goToListBtn =document.getElementById("goToListBtn");
          ...
          if(goToListBtn != null) { // 화면에 목록으로 버튼이 있을때만 수행

              // 함수 선언(변수 대입 형식)
              const goToListFn = ( ) => {

                 //URL 객체 : JS에서 주소를 다루고 관리할 수 있는 객체 
                 // URL객체 .searchParams : 주소에 담긴 쿼리 스트링을 map 형식으로 반환
