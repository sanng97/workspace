...
// 입력 받은 이메일이 정규식과 일치하는 않는 경우 
  ...
  checkObj.memberEmail=true; // 유효하지 않은 상태임을 기록
...
nickMessage.innerText = "메일을 받을 수 있는 이메일을 입력해주세요";
...
if(regEx.test(memberNickname.value)) {
  ...
  nickMessage.innerText="한글,영어,숫자로만 2~10글자 작성해주세요"
  nickMessage.classList.add("error"); 
  nickMessage.classList.remove("confirm"); 
  checkObj.memberNickname=false; 
...
if (memberTel.value.trim().length == 0){
  ...
  memberTel.value ='';
  telMessage.innerText = "메일을 받을 수 있는 이메일을 입력해주세요";
...
document.getElementById("signUpFrm").addEventListener("submit", e => {
  ...
  /* checkObj의 모든 값을 검사해서 하나라도 false이면 가입시도 x*/
    ...
    if( !checkObj[key]){
      ...
      switch(key){
        ...
        case "memberPwConfirm":
          str = "비밀번호가 일치하지않습니다."; break;

          case "memberNickname":
            str = "닉네임이 유효하지않습니다."; break;

            case "authKey":
            str = "인증번호가 유효하지않습니다."; break;
          

            case "memberTel":
              str = "전화번호가 유효하지않습니다."; break;
      }

      alert(str);

      // key == input id 속성 값
      // 유효하지않은 input 태그로 focus 맞춤
...
if(authKey.value.trim().length == 6) {
  authKeyMessage.innerText="유효한 인증번호 입니다.."
  ...
  authKeyMessage.innerText="유효하지 않은 인증번호 입니다."
