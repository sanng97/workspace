...
// x 버튼이 클릭된 input 태그의 순서를 기록할 set생성
// set 특징: 순서 x, 중복 허용 x -> x버튼 여러번 눌러도 하나만 추가
...
boardUpdateFrm.addEventListener('submit', e  => {
  ...
  if (content.value.trim().length==0) {
    ...
    // hidden 타입 태그에 삭제한 이미지 번호 목록 추가 (1,2,3)
  document.querySelector("[name='deleteOrder']").value = Array.from(deleteOrderSet);

   // 수정 성공 시 기존 상세조회 주소가 동일하게 유지 될 수 있도록
   // 쿼리스트링만 별도  input에 저장
