...
@Transactional // 예외 발생 시 롤벡
@Slf4j
@Service // service역할( 비즈니스 로직) +bean등록
public class MemberServicelmpl implements MemberService {
	...
	public Member login(Member inputMember) {
		// 1. 이메일이 일치하는 탈퇴하지 않은 회원 정보 조회 (PW포함)
		Member loginMember = mapper.login(inputMember);
		// 2. 조회 결과가 없을 경우에는 return null;
		log.debug("DB 조회 결과 : " + loginMember);
		
		if(loginMember == null) return null;
		
		// 3.  입력받은 비밀번호와 조회한 비밀번호가 같지않으면 return null;
