...
public class AjaxController {
	...
	public String selectMemberNo(int memberNo) {
		...
		/* 비동기 방식으로 응답하는 방법*/
		// - 비동기 통신은 페이지 갱신 (화면 새로만들기 목적아님)
		// - 단순히 요청에 맞는 응답 데이터만 주고 받는 것이 목적
		// ->>> 조회 결과 email을 요청한 곳으로 반환
		
		return email; 
		
		// 문제가 발생한다 =====> email을 반환 하려는데 controller에서 반환된 값은
		// ViewResolver에 의해 자동으로
		// /templates/반환값.html로 forward를 수행하는 문제 발생
		
		
		//[해결방법]
		// controller의 메서드 반환값이
		// forward를 위한 경로가 아닌 비동기 요청한 곳으로 돌아가야되는 값 자체임을 명시
	...
	@ResponseBody // forward 아니고 값 자체 반환 함 (비동기)
	...
	public Member selectMember(int no){
		...
		//    +JSON이니까 JS객체로 변환해서 사용해 라는 구문을 첨부
		//   ==> @XXXMapping() 구문에 
		// ==>   produces="application/json ; charset=UTP-8"추가
		
		// 3) JS에서 자동으로 JS 객체로 변환되어 사용가능
