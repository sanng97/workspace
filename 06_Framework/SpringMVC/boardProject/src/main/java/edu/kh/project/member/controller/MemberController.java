...
@SessionAttributes({"loginMember"}) // Model의 key중 "loginMember"를 Session으로 이동
public class MemberController {
	
	
	// 등록된 bean중 MemberServiceImpl을 
	// 해당 필드에 의존성 주입
	@Autowired
   ...
   public String login(Member inputMember, Model model
	   ...
	   // - 기본 scope == request scope
	   	...
	   	// ----------------------------------------------
	   	// 로그인 실패 시 특정 메세지를 session에 세팅하여
	   	// 메인페이지에서 alert()로 출력하기
	   	
	   	// *잠깐* session에 세팅하는 이유 :
	   	
	   	// 1) redirect시 request scope 이용불가
	   	// 2) session에 계속 있으면 계속 메세지가 출력되기 떄문에
	   	...
	   	// redirect시 기존 요청(request scope)은 삭제됨
   ...
   public String signup() {
	   // /WEB-INF/view/member/signup.jsp로 forward
   ...
   public String signup(Member inputMember ,
	   ...
	   // 회원가입 서비스 호출 후 결과(INSERT 행의 개수) 반환받기
