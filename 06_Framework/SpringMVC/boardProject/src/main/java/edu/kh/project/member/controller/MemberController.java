...
public class MemberController {
	...
	public String login(HttpServletRequest req) {
		...
		/*  - Aragument Resolver (매개변수 확인자(해결사))
		 * 매개변수에 원하는 타입을 선언하면
...
//	@PostMapping("/login") // RequestMapping의 POST 버전
	...
	public String login(/*@ModelAttribute*/ Member member) {
		 ...
		 * - DTO와 같이 사용하는 어노테이션
		 * 
		 * - DTO의 필드명과 파라미터 KEY 가 같으면 DTO 객체 필드에 자동으로 SET (setter)필요
		 * 
		 * 
		 * 주의사항
		 * - DTO 에 기본 생성자 꼭 작성 (필수)
		 * - DIO에 Setter 꼭 작성!! (필수
		 * 
		 * [1]@ModelAttribute
		 * 
		 ...
		 * 
		 * 
		 * *** @ ModelAttribute 로 인해서 필드에 값이 세팅된 객체를 [커맨드 객체]라고 한다.
		 * 
		 * 
		 * 
		 * 
