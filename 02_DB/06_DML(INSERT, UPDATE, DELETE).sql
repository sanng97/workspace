...
VALUES('D0','개발부','L1'); --1행삽입

SELECT * FROM DEPARTMENT2; -- 확인

ROLLBACK; -- DML 실행취소

SELECT  * FROM DEPARTMENT2; --ROLLBACK 확인
...
-- VALUES -- 서브쿼리로 INSERT시 VALUES 생략
(SELECT EMP_ID,EMP_NAME,DEPT_TITLE
FROM EMPLOYEE 
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID));
-- 23행 삽입

SELECT *  FROM EMP_01; -- 23행 확인
...
-- DEPT_ID가 'D9'인 행의 DEPT_TITLE만 수정
...
SELECT EMP_NAME,BOUNUS FROM EMPLOYEE2;
...
--UPDATE 후 위에 SELECT 수행 -> 결과 확인 후 ROLLBACK


-- EMP LOYEE2 테이블에서 아시아지역에 근무하는 직원의 보너스를 0.3으로 변경
...
SELECT * FROM EMP_M01; 
SELECT * FROM EMP_M02; -- EMP__M01과 겹치는 행 4행, 겹치지 않는 행 1행
...
WHEN NOT MATCHED THEN -- 같은 사번이 존재하지 않으면  INSERT (곽두원)
...
SELECT *FROM EMPLOYEE2;
ROLLBACK; -- DML (INSERT, UPDATE,DELETE) 실행취소
