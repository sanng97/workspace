...
/* 자료형
    ...
    CLOB : 대용량 문자 데이터 (4GB) (Character Long of Byte)
*/

-- MEMBER 테이블 생성

-- 1 )필요한 컬럼 : 아이디, 비밀번호 , 이름, 주민번호, 가입일
-- 2 ) 컬럼명 지정 :MEMBER_ID ,MEMBER_PWD,MEMBER_NAME, 
--              MEMBER_SSN, ENROLL_DATE
...
-- 딕셔너리 뷰 중  USER_TABLES
-- :현재 사용자(계정)가 만든 테이블 목록
...
INSERT INTO MEMBER 
VALUES('MEM01', '123ABC', '홍길동','010101-1234567',DEFAULT);
--TO_DATE('2023-08-27');
--SYSDATE
--DEFAULT ( = SYSDATE)
...
/*
    ...
    NOT NULL(컬럼에 NULL안됨), 
    UNIQUE (중복안됨), CHECK(지정된 값만 삽입, 수정 가능), 
    FOREIGN KEY(다른 테이블의 기본키 또는 UNIQUE가 설정된 컬럼을 참조).
    -> 다른 테이블의 값을 가져다 씀
    -> 두 테이블이 같은 값을 가지는 컬럼을 생기게됨
    -> JOIN 가능 =============== 관계형성
*/
...
CREATE TABLE USER_USED_NN(
    USER_NO NUMBER NOT NULL,  -- 컬럼 레벨 제약 조건 설정 (NOT NULL은 컬럼레벨만 지정가능)
...
INSERT INTO USER_USED_NN
VALUES(1, NULL, NULL, NULL, NULL, '010-1234-5678', 'hong123@kh.or.kr');
...
DROP TABLE USER_USED_UK; -- USER_USED_UK삭제
...
CREATE TABLE USER_USED_UK(
    ...
    -- USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE,
    					...
    					-- 컬럼 레벨 제약 조건( 제약 조건 이름 지정 0)
    
    USER_ID VARCHAR2(20), -- 컬럼레벨제약조건X
    USER_PWD VARCHAR2(30) ,
    USER_NAME VARCHAR2(30),
    GENDER VARCHAR2(10),
    PHONE VARCHAR2(30),
    ...
    --테이블 레벨 제약 조건 (테이블 기본 구조 완성 후 제약 조건 추가)
...
-- ORA-00001 : 무결성 제약조건
--> SYS-C0013063 : 제약조건 이름 미설정 시 자동으로 지정되는 이름
...
-- 컬럼 레벨로 PK 지정
  ...
  --USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, -- 컬럼 레벨, 이름 지정 X
    
	USER_NO NUMBER, --테이블 레벨 지정용 컬럼(테이블 마지막줄에 작성)
    ...
    -- 테이블 레벨
   -- , PRIMARY KEY(USER_NO) -- 테이블 레벨, 이름 지정X
...
-- ORA-02291 : 무결성 제약조건이 위배되었습니다 - 부모키가 없습니다

-- 참조하는 테이블 = 자식테이블
-- 참조를 당하는 테이블 = 부모테이블

/* JOIN : 두 테이블에서 같은 값을 가지고 있는 컬럼을 기준으로 연결
 * ( FK 제약조건이 설정되어 있지 않아도 사용가능)
 * 
 * FK 제약조건이 설정된 부모-자식 테이블
 * - >  무조건 JOIN 가능
 */

--USER_USED_FK , USER_GRADE 테이블 JOIN
SELECT USER_I, USER_NAME, GRADE_CODE, GRADE_NAME
FROM USER_USED_FK
...
DELETE FROM USER_GRADE WHERE GRADE_CODE = 20; --> 삭제 성공

-- USER_USED_FK 테이블에서 참조 중인 값 (GRADE_CODE = 10)을 삭제

DELETE FROM USER_GRADE WHERE GRADE_CODE = 10; --> 삭제 성공

--ORA-02292: 무결성 제약조건(A230724_PSE.GRADE_CODE_FK1)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
CREATE TABLE USER_USED_FK2(
  ...
  REFERENCES USER_GRADE2(GRADE_CODE) ON DELETE SET NULL
                                    /*부모키가 삭제되면 해당 컬럼값을 NULL로 세팅*/
  
  SELECT * FROM USER_GRADE2; --부모테이블
  SELECT * FROM USER_USED_FK2; --자식테이블
...
-- 자식 테이블 조회
SELECT * FROM USER_USED_FK2;
--> 홍길동, 이순신의 GRADE_CODE가 10 - > NULL 변화 확인
...
CREATE TABLE USER_USED_FK3(
  ...
  REFERENCES USER_GRADE3 -- 참조할 컬럼을 작성하지 않으면 PK를 자동참조
  ON DELETE CASCADE
...
SELECT * FROM USER_GRADE3; -- 10 삭제확인
...
--SQL Error [2290] [23000]: ORA-02290: 체크 제약조건(A230724_PSE.GENDER_CHECK)이 위배되었습니다
-- GENDER 컬럼에 CHECK 제약조건으로 남 또는 여 만 기록 가능인데 남자라는 조건때문에 안됨
...
UPDATE USER_USED_CHECK
SET GENDER = '여자' -- '남' 또는  '여 '만 기록가능
WHERE USER_NO = 1; -- 실패

-- ORA-02290: 체크 제약조건(A230724_PSE.GENDER_CHECK)이 위배되었습니다
...
CREATE TABLE USER_TEST(
  ...
  USER_PWD VARCHAR(100) CONSTRAINT NN_USER_PWD NOT NULL,
  PNO CHAR(14) CONSTRAINT NN_PNO NOT NULL /*CONSTRAINT UK_PNO UNIQUE*/,
  GENDER CHAR(3) CONSTRAINT CK_GENDER CHECK(GENDER IN ('남','여')),
 -- GENDER NVARCHAR2(1), -- 유니코드 글자 수 1개
  PHONE VARCHAR2(20),
...
SELECT * FROM EMPLOTEE_COPY;

--> 복사한 테이블에 PK,CHECK,FK,UNIQUE 복사 X
-- DEFAULT 값, COMMENT도 복사X

-- 2) JOIN 후 원하는 컬럼만 테이블로 복사

CREATE TABLE EMP_COPY2
AS SELECT EMP_NAME, NVL(DEPT_TITLE, '소속없음') AS DEPT_TITLE ,JOB_NAME
FROM EMPLOYEE
...
-- 3)테이블의 형태 (컬럼명, 데이터 타입)만 복사하기
--> 데이터가 없는 빈 테이블로 복사하기
--> 조건식 결과를 무조건 FALSE로 만들면 가능

CREATE TABLE DEPATMENT_COPY
AS SELECT * 
FROM DEPARTMENT
-- WHERE DEPT_ID = 'D1';
WHERE 1= 0;
...
ALTER TABLE EMPLOYEE  ADD CONSTRAINT DEPT_CODE_FK
FOREIGN KEY(DEPT_CODE) REFERENCES DEPARTMENT /* (DEPT_ID)*/;
                          --> 참조 테이블 컬럼명 생략-> 자동으로 PK와 연결
...
ALTER TABLE EMPLOYEE ADD -- 무언가 추가해서 EMPLOYEE 테이블 변경할꺼
CONSTRAINT SAL_GRADE_FK -- 제약조건 이름 지정
FOREIGN KEY(SAL_LEVEL) REFERENCES SAL_GRADE;
       -- FK 제약조건 추가(EMPLOYEE.SAL_LEVEL 컬럼이 SAL_GRADE의 PK를 참조하겠다)
