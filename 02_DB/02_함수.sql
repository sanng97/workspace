...
SELECT TO_CHAR(SYSDATE, 
	'YYYY"년" MM"월" DD"일" DAY PM HH"시" MI"분" SS"초"')
FROM DUAL;
-- ORA-01821: 날짜 형식이 부적합합니다
--> 년,월,일,시,분,초 는 오라클에서 지정항 시간 관련 패턴이 아니다!
...
SELECT TO_NUMBER('$1,500', '$9,999') FROM DUAL; -- 1500
...
--SELECT NULL / 10 FROM DUAL; --> NULL 산술 연산 결과는 항상 NULL

SELECT EMP_NAME, SALARY * 12 "연봉", 
	NVL(SALARY * 12 * BONUS, 0) "인센티브"
FROM EMPLOYEE
ORDER BY 2 DESC;
...
SELECT ROUND( AVG(SALARY) ) 급여평균 FROM EMPLOYEE;
...
-- 전화번호가 있는 사원은 몇명인지 조회
SELECT COUNT(PHONE) FROM EMPLOYEE; -- 20

SELECT COUNT(*) FROM EMPLOYEE
WHERE PHONE IS NOT NULL; -- PHONE 컬럼이 NULL이닌 행만 카운트
