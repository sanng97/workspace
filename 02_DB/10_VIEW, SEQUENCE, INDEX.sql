...
-- 모든 사원의 사번, 이름, 부서명, 직급명을 사번 오름차순으로 조회
...
-- 지역명에  ASIA가 포함되는 직원 사번 내림 차순 조회
SELECT * FROM V_EMP
WHERE 지역명 LIKE '%ASIA%'
ORDER BY 사번 DESC;
...
-- D0 / NULL / L2
--> VIEW에 삽입을 시도했는데 원본 테이블에 삽입
--> VIEW는 개별적인 저장공간이 있는 테이블이 아니라
--> 원본 테이블의 필요한 컬럼, 행을 모아서 보여주는 가상의 테이블
--> NULL이 삽입 == 무결성을 약하게 만드는 원인
...
INCREMENT BY 5 -- NEXTVAL 마다 5씩 증가
MAXVALUE 150 -- 증가 가능한 최대값 150


NOMINVALUE --최소값 없음
NOSYCLE -- 최대값 까지 증가 후 반복 X
CACHE -- 미리 발생시킬 숫자를 만들어 둠
...
-- TB_TEST 테이블에서 TEST_NO가 가장 작은 컬럼 값을
-- 다음 스퀀스 번호로 수정
...
SELECT * FROM TB_TEST;
-- MAXVALUE은 사례로 될수없습니다 ->MAXVALUE 초과

--------------------------------

-- SEQUENCE 변경(ALTER)

/* R기본적으로 CREATE 작성법과 같으나 START WITH는 제외
 * 
 [작성법]
...
WHERE TEST_ID = 'TEST500000'; -- 20/18/21/19MS

-- 인덱스 사용 O
SELECT * FROM TB_INDEX_TEST
WHERE TEST_NO = 500000; --3/1/2/0
