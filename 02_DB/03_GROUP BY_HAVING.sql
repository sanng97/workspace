...
-- 부서별(같은 부서끼리 그룹을 만들어) 급여 합 조회
SELECT DEPT_CODE, SUM(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE; -- 같은 부서끼리 그룹을 만들어라


/* SELECT문 해석 순서 (무조건 외워라!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!)
...
-- 부서코드 순으로 정렬
SELECT DEPT_CODE, COUNT(BONUS) /* BONUS가 NULL인 행 빼고 카운트 */
FROM EMPLOYEE 
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE;
...
FROM EMPLOYEE 
WHERE DEPT_CODE IN('D5', 'D6') -- 부서코드가 'D5','D6'인 행만 남음
GROUP BY DEPT_CODE; -- 'D5', 'D6' 그룹으로 나뉨
...
-- 부서별 급여 평균 3000000원 이상인 부서를 조회하여 부서코드 오름차순으로 정렬
