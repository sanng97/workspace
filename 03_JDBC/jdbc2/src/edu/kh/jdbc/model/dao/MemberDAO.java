...
//				+ "	DEFAULT, DEFAULT)";
		...
		try {
			...
			// (Statement와 다르게 생성 시 sql을 매개변수로 사용함)
	...
	public int deleteMember(Connection conn, Member member) {
		...
		// 1. 최종 결과 저장 변수 선언(무조건해야함 암거나 예외 없음 무조건 무조건 무조건)
		int result = 0;
	 ...
	 * @return
	 */
	public int newPw(Connection conn, Member member) {
		int result = 0;
		
		String sql = "UPDATE MEMBER"
				+ "SET MEMBER_EMAIL = ? AND MEMBER_PW = ?";
