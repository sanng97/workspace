...
inputId.addEventListener("input", e => {
  ...
  if(val.length === 0){ // 입력된 값이 없을 경우
    ...
    checkList["input-id"] = false; // 유효성 검사 여부 저장 객체에서 "input-id"의 값을 false 변경 (유효하지 않다를 뜻함)
...
inputPw.addEventListener("input",e => {
  ...
  e.target.value = val; // 공백이 제거된값을 input값으로 대입

  if(val.length===0){
    span.innerText = "8글자 이상 20글자 이하";
    span.classList.remove=("check");
    span.classList.remove=("error");
...
if(!pwSpan.classList.contains("check")){
  ...
  if(val=== pwVal){ // 비밀번호/ 비밀번호 확인이 같을 경우
