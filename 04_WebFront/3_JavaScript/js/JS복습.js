...
createBtn.addEventListener("click", e => {
  ...
  // 생성 버튼의 이전형제 요소 (input)의 값 얻어오기
  const bingoLength = e.target.previousElementSibling.value;


  // bingoLength * bingoLength 한 수 만큼 중복 되지 않는 난수 생성

  // java 컬렉션 중 set (중복 x)과 똑같은 객체 존재!!!!!!!
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    col.addEventListener("click", e => {
      ...
      if (e.target.classList.contains("check")) {
        ...
        e.target.classList.remove("check"); // check클래스제거
      } else {
...
function checkBingo() {
  ...
  for (let i = 0; i < rows.length; i++) {
    // rows[i].children : 각 행의 모든 열 (자식요소)을 배열로 반환
...
function rowCheck(board) {
  ...
  for (let i = 0; i < board.length; i++) {
    // 한 행 씩 접근해서 모든 열에 check클래스가 있는지 검사
    let count = 0; // 현재 행에 check 개수 count

    // 모든 열 접근용 for문
    for (let k = 0; k < board[i].length; k++) {

      // 열에 check 클래스가 있다면 count 증가
...
function colCheck(board) {
  ...
  // board[k][i] : i번째 행의 k번째 열

  // board[0].length ==  0행의 열의길이
  // 빙고판은 정사각형 == 행과 열의 길이 같다

  const bingoLength = board.length; // 빙고판 길이
  for (let k = 0; k < board.length; k++) { // 열제어
    let count = 0; // check 개수 count

    for (let i = 0; i < bingoLength; i++) {  // 행제어
      if (board[i][k].classList.contains("check")) {
        count++;
      }
    }

    if (bingoLength == count) { // 모든 열이 체크되어 있다면
      for (let i = 0; i < bingoLength; i++) {  // 행제어
        board[i][k].classList.add("bingo");
      }
    }
...
function diaCheck(board) {
  ...
  let count2 = 0; // 오른쪽 위 ->  왼쪽아래 대각선 카운트

  const bingoLength = board.length; // 빙고판길이
