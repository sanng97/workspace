...
check2.addEventListener('click', () => {
  ...
  let regEx = /java/; // 문자열 내에 java가 존재하는 검색

  console.log(`javascript : ${regEx.test("javascript")}`); // true
  console.log(`jajajaja : ${regEx.test("jajajaja")}`); // false
...
inputPhone.addEventListener("keyup", () => {
  ...
  if (value.length == 0) { // 입력 X
    phoneResult.innerText = "phoneResult";
  ...
  if (regEx.test(value)) { // 유효한 경우
    ...
    phoneResult.classList.remove("error"); // error 클래스 제거

  } else { // 유효하지 않은 경우
    phoneResult.innerText = "유효하지 않은 형식입니다";
    phoneResult.classList.add("error"); // error 클래스 추가
