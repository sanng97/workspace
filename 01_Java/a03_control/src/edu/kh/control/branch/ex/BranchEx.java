...
public class BranchEx {
	...
	//분기문
	//break : 반복문을 멈춤
	// continue : 반복문의 시작부분으로 이동( 다음반복진행)
	
	public void ex1() {
		for ( int i = 1; i<=10000; i++) {
			
			if ( i% 2 ==1) { //홀수인경우 다음 반복으로 넘어감
				continue;
			}
...
//		System.out.println("합계 : " + sum);	
	...
	public void ex3() {
		for (int row=  1; row <=5; row ++) {
			...
			// row가 3일 때 반복 종료
				if(row ==3) {
					break; //break를 감싸고 있는  row제어 for문이 멈춤
				}
		}
	...
	public void upDownGame() {
		...
		// 틀린경우 입력한 값이 난수보다 큰지, 작은지 출력
		
		Scanner sc = new Scanner(System.in);
		
		//Math.random() : 0.0이상 1.0 미만의 난수 반환

		int answer = (int) (Math.random() * 100 + 1); // 1~100
		
		//System.out.println(answer); // 임시로 답안 확인
		
		int count = 1; // 입력횟수 저장용
