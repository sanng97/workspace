...
public class CarRun {
	public static void main(String[] args) {
		...
		// 부모 참조 변수 = 자식 객체 생성
		Car c2 = new Porsche(); // 오잉? 왜 에러가 발생 안하지?
...
//_---------------------------------------------------------------------------
		...
		// Poly Service 객체생성
		 ...
		 for(int i =0; i<6; i++) { //ctrl +shift +f: 자동정렬 (정확도 x)
		 
		Car car1  = service.selectOne(i); // arr에서 i번째 인덱스 얻어오기
		System.out.println("================================== "); 
		
		
		 if (car1 != null) { // 인덱스 정상범위
			 
			 System.out.println("바퀴 수 : " +  car1.getWheel());
			 System.out.println(" 좌석 수 :"+ car1.getSeat());
			 System.out.println("연료 : "+ car1.getWheel());
			...
			//spark 인 경우 할인율?
			 ...
			 System.out.println(" 뚜껑열림 ? "+((Porsche)car1).isOpenTheDoor());
			 ...
			 //ClassCastException 
			 //- > 올바르지 않은 자료형으로 형변환시 발생
			 // -> spark 객체를 참조하던 Car1을 Porsche 타입으로변경
