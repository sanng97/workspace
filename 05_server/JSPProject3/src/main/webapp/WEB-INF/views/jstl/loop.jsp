...
<body>
    ...
    <h${num}>현재 num값 : ${num}</h${num}>
    </c:forEach>

  <h3 style ="color:red">c:forEach는 step은 양수만 가능</h3>

  <%-- h6 -> h1까지 반복 출력 --%>
    <c:forEach var="num" begin="1" end="6" step ="1">
    <h${7-num}>현재 num값 : ${num}</h${num}>
    </c:forEach>

  <hr>
  <h3>boardList 일반 for문으로 다루기</h3>
  <pre>
   접두사 fn (JSTL-EL)
   -fn(function, 기능 ) : 문자열, 컬렉션 관련 메서드를 
    ...
    - \${fn:length(배열 또는 컬레션 또는 문자열)} : 길이변환
  </pre>
