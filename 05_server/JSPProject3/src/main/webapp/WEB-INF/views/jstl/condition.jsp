...
<body>
  ...
  <pre>
    ...
    ** test 속성값 작성 시 주의사항 **
    ...
    2. true/false가 최종 결과로 나와야 한다.

    3. 속성 값을 작성하는  "" 제일 앞/뒤에는 띄어쓰기가
       있으면 안된다

       오류 예시 : test="    /${key=123}" // 앞공백
       오류 예시 : test="/${key=123}       "  // 뒷공백
  ...
  <c:set var="price" scope="page" value="9500"/>
...
-->
  ...
  ***  c:choose 태그 작성 주의 사항 ****
  choose 태그 내부에는 
  c:when , c:oterwise , JSP 주석, 공백 이외에는 아무것도 작성되면 안된다.


  - c:when (언제) 태그 : if,else if를 나타내는 태그
                  속성은 test밖에 없다 (c:if 참고)

  - c:otherwise(그렇지 않으면) 태그
  : else 역할의 태그로 아무런 속성이 없다.
  </pre>
  <hr>

  <h4>테스트 방법</h4>
  <pre>
  주소창에 값을 직접 입력한 후 \${param.key}를 이용해 값을 얻어오기
